...
export function MonografTab() {
  ...
  useEffect(() => {
    async function fetchPaginatedMonografs() {
      try {
        setLoading(true);
        const data = await dataService.getMediaItems('monograf', {
          page: currentPage,
        ...
        console.error("Failed to fetch monografs:", error);
      } finally {
        setLoading(false);
      }
    }
    fetchPaginatedMonografs();
      ...
      <div className="flex flex-col gap-8">
        <div className="flex flex-col md:flex-row items-center justify-between gap-6">
          <div className="flex w-full md:w-auto items-center">
            <h2 className="text-4xl font-extrabold text-[#092C74] title-font uppercase">Monograf</h2>
          </div>
        ...
        <div className="flex flex-col md:flex-row items-center justify-between gap-4">
          <div className="flex flex-wrap items-center gap-4 w-full md:w-auto">
            <div className="flex items-center gap-3 bg-white rounded-full px-6 py-2 shrink-0 w-full sm:w-[200px] border border-[#092C74] text-[#092C74]">
            ...
            <Popover>
              <PopoverTrigger asChild>
                <button
                  className="flex items-center gap-3 bg-white border border-[#092C74] rounded-full px-6 py-3 shrink-0 font-semibold z-10 relative cursor-pointer outline-none w-full sm:w-[200px] text-[#092C74]"
                >
      ...
      <div className={isGridView ? "grid grid-cols-1 lg:grid-cols-2 xl:grid-cols-3 gap-8" : "space-y-8 max-w-5xl mx-auto"}>
        ...
        {paginatedData.length === 0 && (
          <div className="py-20 text-center col-span-full">
             ...
             <p className="text-gray-500 font-bold uppercase tracking-widest">Tidak ada monograf yang ditemukan</p>
          </div>
        )}
