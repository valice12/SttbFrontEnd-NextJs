...
import { AcademicDto, CurriculumCategory, Lecture } from '@/lib/types';
...
export function AkademikDetail() {
  ...
  return (
    <div className="min-h-screen bg-gray-50 pb-20">
      ...
      {/* Main Info Cards */}
      <section className="-mt-8 sm:-mt-12 relative z-20 container mx-auto px-4 mb-24 md:mb-32">
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6 md:gap-8">
          <motion.div
            initial={{ opacity: 0, y: 30 }}
            animate={{ opacity: 1, y: 0 }}
            transition={{ delay: 0.1 }}
            className="bg-white/70 backdrop-blur-3xl p-8 sm:p-12 rounded-[2.5rem] md:rounded-[3.5rem] shadow-[0_40px_80px_rgba(9,44,116,0.08)] flex flex-col items-center text-center border border-white group md:hover:translate-y-[-10px] transition-all duration-700"
          >
            <div className="size-16 sm:size-24 bg-[#092C74]/5 rounded-2xl sm:rounded-[2rem] flex items-center justify-center mb-6 sm:mb-8 shadow-inner group-hover:bg-[#092C74] md:group-hover:rotate-[15deg] transition-all duration-700">
              <BookOpen className="size-8 sm:size-12 text-[#092C74] group-hover:text-white transition-colors" />
            </div>
            <h3 className="font-black text-[10px] tracking-[0.2em] uppercase text-gray-400 mb-3 sm:mb-4 tracking-widest">Jumlah Kredit</h3>
            <p className="text-4xl sm:text-5xl font-black text-[#092C74] tracking-tighter mb-2">{programData.totalCredits || (programData as any).totalCredit}</p>
            <p className="text-[10px] sm:text-sm font-black text-[#E31D1A] uppercase tracking-widest">Satuan Kredit Semester</p>
          </motion.div>
          ...
          <motion.div
            ...
            className="bg-white/70 backdrop-blur-3xl p-8 sm:p-12 rounded-[2.5rem] md:rounded-[3.5rem] shadow-[0_40px_80px_rgba(9,44,116,0.08)] flex flex-col items-center text-center border border-white group md:hover:translate-y-[-10px] transition-all duration-700"
          >
            <div className="size-16 sm:size-24 bg-[#E31D1A]/5 rounded-2xl sm:rounded-[2rem] flex items-center justify-center mb-6 sm:mb-8 shadow-inner group-hover:bg-[#E31D1A] md:group-hover:rotate-[15deg] transition-all duration-700">
              <Clock className="size-8 sm:size-12 text-[#E31D1A] group-hover:text-white transition-colors" />
            </div>
            <h3 className="font-black text-[10px] tracking-[0.2em] uppercase text-gray-400 mb-3 sm:mb-4 tracking-widest">Masa Studi</h3>
            <p className="text-4xl sm:text-5xl font-black text-[#092C74] tracking-tighter mb-2">{programData.duration.split(' ')[0]}</p>
            <p className="text-[10px] sm:text-sm font-black text-[#E31D1A] uppercase tracking-widest">{programData.duration.split(' ').slice(1).join(' ') || 'Semester'}</p>
          </motion.div>
          ...
          >
            <div className="size-16 sm:size-24 bg-[#092C74]/5 rounded-2xl sm:rounded-[2rem] flex items-center justify-center mb-6 sm:mb-8 shadow-inner group-hover:bg-[#092C74] md:group-hover:rotate-[15deg] transition-all duration-700">
              <Target className="size-8 sm:size-12 text-[#092C74] group-hover:text-white transition-colors" />
            </div>
            <h3 className="font-black text-[10px] tracking-[0.2em] uppercase text-gray-400 mb-3 sm:mb-4 tracking-widest">Ethos</h3>
            <p className="text-xl sm:text-2xl font-black text-[#092C74] italic leading-tight px-2 sm:px-4 group-hover:scale-105 transition-transform duration-500">
              "{programData.motto}"
            </p>
          </motion.div>
        </div>
