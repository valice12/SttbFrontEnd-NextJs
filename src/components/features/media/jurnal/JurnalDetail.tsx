      ...
      <div className="bg-white border-b border-gray-100 mb-4">
        <div className="container mx-auto px-4 py-4 flex flex-col sm:flex-row justify-between items-center gap-3">
          <div className="flex items-center gap-2 text-[10px] sm:text-xs font-bold text-gray-500 uppercase tracking-widest truncate w-full sm:w-auto">
            ...
            <span className="text-[#092C74] truncate">{journal.journalTitle || journal.title}</span>
          </div>
              ...
              <div className="mb-10">
                <div className="flex items-center gap-3 mb-6">
                  ...
                  <span className="text-[10px] sm:text-xs text-gray-400 font-bold uppercase tracking-tighter">
                    {journal.publicationDate ? new Date(journal.publicationDate).toLocaleDateString('id-ID', { year: 'numeric', month: 'long', day: 'numeric' }) : journal.date}
                  </span>
                </div>
                  ...
                  <div className="flex items-center gap-2">
                    ...
                    <span className="text-xs sm:text-sm font-bold text-gray-900">
                      {journal.authors?.length > 0
                        ? journal.authors.map((a: any) => a.authorName).join(', ')
                        : (journal.authorName || 'Tim Akademik STTB')}
                    </span>
                  </div>
                  {/* <div className="flex items-center gap-2">
              ...
              <div className="mt-16 pt-10 border-t border-gray-100">
                ...
                <div className="bg-gray-50 p-6 rounded-2xl border border-gray-100 relative group overflow-hidden">
                  ...
                  <code className="text-sm text-gray-600 block leading-relaxed break-all">
                    {journal.authors?.length > 0 
                      ? journal.authors.map((a: any) => a.authorName).join(', ') 
                      : (journal.authorName || 'Tim Akademik STTB')}. ({journal.publicationDate ? new Date(journal.publicationDate).getFullYear() : new Date().getFullYear()}). {journal.journalTitle || journal.title}. <span className="italic">Jurnal Teologi Transformatio</span>, Volume 22(1). STT Bandung Press.
                  </code>
                  <div className="mt-4 flex justify-end">
                    <Button 
                      variant="ghost" 
                      size="sm" 
                      className="text-[10px] font-black uppercase text-[#1C64E8]"
                      onClick={() => {
                        const citation = `${journal.authors?.length > 0 ? journal.authors.map((a: any) => a.authorName).join(', ') : (journal.authorName || 'Tim Akademik STTB')}. (${journal.publicationDate ? new Date(journal.publicationDate).getFullYear() : new Date().getFullYear()}). ${journal.journalTitle || journal.title}. Jurnal Teologi Transformatio, Volume 22(1). STT Bandung Press.`;
