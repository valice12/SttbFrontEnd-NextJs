    ...
    <div className="min-h-screen bg-white">
          ...
          <div className="grid lg:grid-cols-2 gap-16 items-start">
            ...
            >
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-6">
                {[
                  ...
                  <motion.div 
                    key={i}
                    whileHover={{ y: -5 }}
                    className="bg-white/70 backdrop-blur-md p-6 md:p-8 rounded-[2rem] border border-white shadow-[0_20px_40px_rgba(9,44,116,0.05)] group"
                  >
                    <div className={`size-12 md:size-14 bg-gradient-to-br ${item.color} rounded-2xl flex items-center justify-center mb-6 shadow-xl group-hover:rotate-12 transition-transform`}>
                      <item.icon className="size-5 md:size-6 text-white" />
                    </div>
                    <h3 className="font-black text-lg md:text-xl text-[#092C74] mb-3 tracking-tight">{item.title}</h3>
                    <p className="text-gray-500 text-sm md:text-base font-medium whitespace-pre-line leading-relaxed italic">{item.desc}</p>
                  </motion.div>
                ))}
              </div>
              ...
              <div className="p-8 md:p-10 bg-[#092C74] rounded-[2.5rem] text-white relative overflow-hidden mt-8 shadow-2xl">
                 ...
                 <h4 className="text-xl md:text-2xl font-black mb-4 relative z-10">Kunjungi Kampus</h4>
                 <p className="text-sm md:text-base text-white/70 relative z-10 leading-relaxed font-medium">
              ...
              <div className="bg-[#092C74] p-8 md:p-14 rounded-[3rem] md:rounded-[4rem] shadow-2xl relative overflow-hidden group">
                ...
                <form className="space-y-6 relative z-10">
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                    <div>
                      <label className="block text-[10px] md:text-xs font-black text-white/50 uppercase tracking-widest mb-3 ml-2">Nama Lengkap</label>
                      <Input placeholder="Nama Anda" className="h-14 md:h-16 bg-white/5 border-white/10 text-white rounded-2xl focus:bg-white/10 transition-all placeholder:text-white/20" />
                    </div>
                    <div>
                      <label className="block text-[10px] md:text-xs font-black text-white/50 uppercase tracking-widest mb-3 ml-2">Email</label>
                      <Input type="email" placeholder="email@example.com" className="h-14 md:h-16 bg-white/5 border-white/10 text-white rounded-2xl focus:bg-white/10 transition-all placeholder:text-white/20" />
                    </div>
                  </div>
                  <div>
                    <label className="block text-[10px] md:text-xs font-black text-white/50 uppercase tracking-widest mb-3 ml-2">Topik Pesan</label>
                    <Input placeholder="Contoh: Pertanyaan Admisi" className="h-14 md:h-16 bg-white/5 border-white/10 text-white rounded-2xl focus:bg-white/10 transition-all placeholder:text-white/20" />
                  </div>
                  <div>
                    <label className="block text-[10px] md:text-xs font-black text-white/50 uppercase tracking-widest mb-3 ml-2">Pesan Anda</label>
                    <Textarea
                      placeholder="Tulis pesan Anda secara detail..."
                      className="min-h-[150px] md:min-h-[180px] bg-white/5 border-white/10 text-white rounded-3xl focus:bg-white/10 transition-all placeholder:text-white/20 p-5"
                    />
                  </div>
                  <Button className="w-full h-16 md:h-20 bg-white hover:bg-[#E31D1A] text-[#092C74] hover:text-white font-black text-lg md:text-xl rounded-2xl shadow-2xl transition-all duration-500 uppercase tracking-widest group">
                    KIRIM PESAN <Send className="ml-3 size-4 md:size-5 group-hover:translate-x-2 transition-transform" />
                  </Button>
                </form>
              </div>
            </motion.div>
      ...
      <section className="pb-16 md:pb-32 bg-white">
        <div className="container mx-auto px-4">
          <div className="group relative rounded-[2.5rem] md:rounded-[4rem] overflow-hidden shadow-2xl h-[400px] md:h-[500px] border-[6px] md:border-8 border-white">
            ...
            <div className="absolute bottom-6 md:bottom-10 left-6 md:left-10 z-20 max-w-[calc(100%-48px)]">
               <div className="bg-white p-4 md:p-6 rounded-[1.5rem] md:rounded-[2rem] shadow-2xl flex items-center gap-4 border border-gray-100">
                  <div className="size-10 md:size-12 bg-[#092C74] rounded-xl flex items-center justify-center shrink-0">
                     <MapPin className="size-5 md:size-6 text-white" />
                  </div>
                  <div className="min-w-0">
                     <p className="font-bold text-[#092C74] text-sm md:text-base truncate">Temukan Kami di Google Maps</p>
                     <p className="text-[10px] text-gray-400 font-black uppercase tracking-widest truncate">Petunjuk Arah & Lokasi</p>
                  </div>
               </div>
            </div>
          </div>
