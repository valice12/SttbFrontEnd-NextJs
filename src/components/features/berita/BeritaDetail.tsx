      ...
      <article className="container mx-auto px-4 py-12 max-w-5xl">
        ...
        >
          ...
          <header className="mb-8 md:mb-12">
            <span className="inline-block px-4 py-1.5 bg-[#E31D1A] text-white rounded-full text-[10px] sm:text-xs font-bold uppercase tracking-wider mb-6">
              {Array.isArray(newsItem.category) ? newsItem.category.join(', ') : (newsItem.category || 'Berita')}
            </span>
            <h1 className="text-3xl sm:text-4xl md:text-5xl lg:text-6xl font-extrabold text-[#092C74] mb-8 leading-[1.15] tracking-tight">
              {newsItem.title}
            </h1>
            
            <div className="flex flex-col sm:flex-row flex-wrap items-start sm:items-center gap-6 sm:gap-8 text-gray-600 border-y py-6 border-gray-100">
              <div className="flex items-center gap-3">
                <div className="size-10 bg-[#092C74]/10 rounded-full flex items-center justify-center text-[#092C74] shrink-0">
                  <User className="size-5" />
                </div>
                <div>
                  <p className="text-[10px] text-gray-400 font-medium uppercase tracking-tighter">Penulis</p>
                  <p className="text-sm font-bold text-gray-900">{newsItem.author || 'Admin STTB'}</p>
                </div>
              </div>
              
              <div className="flex items-center gap-3">
                <div className="size-10 bg-[#E31D1A]/10 rounded-full flex items-center justify-center text-[#E31D1A] shrink-0">
                  <Calendar className="size-5" />
                </div>
                <div>
                  <p className="text-[10px] text-gray-400 font-medium uppercase tracking-tighter">Tanggal</p>
          ...
          {/* Featured Image */}
          <div className="aspect-[4/3] sm:aspect-[16/9] md:aspect-[21/9] w-full bg-gray-100 rounded-3xl overflow-hidden mb-12 shadow-2xl relative">
            <img 
            ...
            <div className="lg:w-2/3">
              <div className="prose prose-sm sm:prose-base md:prose-lg prose-slate max-w-none">
                {newsItem.description && (
                  <p className="text-lg sm:text-xl text-gray-700 leading-relaxed font-medium mb-8 border-l-4 border-[#092C74] pl-4 sm:pl-6 py-2 italic bg-gray-50 rounded-r-lg">
                    {newsItem.description}
                  </p>
                )}
